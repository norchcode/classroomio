<script lang="ts">
  ...
  async function handleSaveCustomDomain() {
    ...
    if (sanitizedDomain.includes(import.meta.env.VITE_APP_HOST || 'classroomio.com')) {
  ...
  <Row class="border-bottom-c py-7">
    ...
    <Column sm={2} md={6} lg={8}>
      ...
      <div>
        ...
        <TextField
          label="URL"
          helperMessage={`https://${siteName || ''}.${import.meta.env.VITE_APP_HOST || 'classroomio.com'}`}
          bind:value={siteName}
          type="text"
      ...
      <div class="border-bottom-c pb-10">
        ...
        {:else}
          ...
          <TextField
            ...
            type="text"
            placeholder={`courses.${import.meta.env.VITE_APP_HOST || 'yourwebsite.com'}`}
            helperMessage={`https://${customDomain || `course.${import.meta.env.VITE_APP_HOST || 'yourwebsite.com'}`}`}
